# 009: Evidence Journal（基本版）

## 概要

画像・URL・メモを証拠（Evidence）として保存する基本機能を実装する。Day21のCommitment Reportで表示するための最小限の実装。

## Phase

**Phase A: MVP**

## 優先度

高

## 依存関係

- 前提: 001 基盤構築, 006 記憶システム
- 後続: 007 Day21 Judgment Gate

---

## 機能要件（MVP版）

### 1. Evidence タイプ（MVP）

| タイプ | 説明 | 例 |
|-------|------|-----|
| 画像（Image） | スクリーンショット、写真 | 完成したコードの画面 |
| URL | 外部リンク | デプロイしたサイト |
| メモ（Note） | テキストメモ | 気づき、学び |

> 音声Evidence は Phase B で追加

### 2. 基本メタデータ

- タイトル（必須）
- 日付（自動）
- タイプ（image/url/note）

> タグ、関連コミットメントは Phase B で追加

### 3. シンプルな一覧表示

- 日付順に証拠を表示
- タイプ別アイコン
- サムネイル表示（画像の場合）

### 4. Day21連携

- Evidence一覧からハイライト3つをAIが選定
- Commitment Reportに表示

---

## UI設計（MVP版）

### Evidence追加ボタン（ホームから）

```
┌─────────────────────────────┐
│  証拠を追加                  │
├─────────────────────────────┤
│                              │
│  [📸 画像] [🔗 URL] [📝 メモ] │
│                              │
│  タイトル:                   │
│  [________________]          │
│                              │
│  内容:                       │
│  [________________]          │
│  (URLの場合はURL入力)        │
│  (メモの場合はテキスト入力)   │
│                              │
│           [保存する]          │
└─────────────────────────────┘
```

### Evidence一覧画面

```
┌─────────────────────────────┐
│  Evidence Journal   [+追加]  │
├─────────────────────────────┤
│                              │
│  📅 2024年1月10日            │
│  ┌───────────────────────┐  │
│  │ 📸 朝5時起床達成       │  │
│  │    [サムネイル]        │  │
│  └───────────────────────┘  │
│                              │
│  📅 2024年1月8日             │
│  ┌───────────────────────┐  │
│  │ 🔗 ブログ記事公開       │  │
│  │    https://...          │  │
│  └───────────────────────┘  │
│                              │
│  📅 2024年1月5日             │
│  ┌───────────────────────┐  │
│  │ 📝 誘惑に勝った         │  │
│  │    YouTubeを見たくなっ   │  │
│  │    たけど我慢した       │  │
│  └───────────────────────┘  │
│                              │
└─────────────────────────────┘
```

---

## データモデル

### evidences テーブル

```sql
CREATE TABLE evidences (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES users(id) NOT NULL,
  type VARCHAR(50) NOT NULL, -- 'image' | 'url' | 'note'
  title VARCHAR(200) NOT NULL,
  content TEXT, -- URL or note content
  file_url VARCHAR(500), -- for image
  date DATE DEFAULT CURRENT_DATE,
  created_at TIMESTAMP DEFAULT NOW()
);
```

---

## API設計

### GET /api/evidences

Evidence一覧を取得

**Query Parameters:**
- `limit`: 件数（デフォルト: 20）

**Response:**
```json
{
  "evidences": [
    {
      "id": "uuid",
      "type": "image",
      "title": "朝5時起床達成",
      "file_url": "https://...",
      "date": "2024-01-10"
    }
  ]
}
```

### POST /api/evidences

Evidenceを追加

**Request (multipart/form-data for image):**
```
type: "image"
title: "朝5時起床達成"
file: (binary)
```

**Request (JSON for url/note):**
```json
{
  "type": "url",
  "title": "ブログ記事公開",
  "content": "https://..."
}
```

### GET /api/evidences/highlights

Day21用のハイライトを取得（AI選定）

**Response:**
```json
{
  "highlights": [
    { "id": "uuid", "title": "...", "date": "...", "reason": "..." }
  ]
}
```

---

## ファイルアップロード

### Supabase Storage

```typescript
async function uploadEvidence(
  userId: string,
  file: File
): Promise<string> {
  const fileName = `${userId}/${Date.now()}_${file.name}`;

  const { data, error } = await supabase.storage
    .from('evidences')
    .upload(fileName, file);

  if (error) throw error;

  const { data: urlData } = supabase.storage
    .from('evidences')
    .getPublicUrl(fileName);

  return urlData.publicUrl;
}
```

---

## Todo

### データモデル
- [x] evidences テーブル作成
- [x] RLS設定
- [x] Supabase Storage バケット作成（手動作業必要）

### 基本機能
- [x] Evidence追加モーダル
- [x] 画像アップロード機能
- [x] URL保存機能
- [x] メモ保存機能

### 一覧表示
- [x] Evidence一覧画面
- [x] 日付順ソート
- [x] サムネイル表示

### API実装
- [x] GET /api/evidences（直接Supabase SDK使用）
- [x] POST /api/evidences（直接Supabase SDK使用）
- [x] POST /api/evidences/analyze（AIスコアリング）

### Day21連携
- [x] ハイライト選定ロジック（AI）
- [ ] Commitment Reportへの表示（Ticket 007で実装）

### 実装メモ
- API実装は直接Supabase SDKを使用する方針（RLS-safe）
- POST /api/evidences/analyze のみサーバーサイドAPI（OpenAI呼び出しのため）
- 画像アップロードは expo-image-picker + Supabase Storage
- リアルタイム更新はSupabase Real-time Subscriptionで実装

---

## 完了条件

1. 画像・URL・メモをEvidenceとして追加できる
2. 一覧画面で日付順に表示される
3. Day21でハイライト3つが表示される
4. 画像がSupabase Storageに保存される
